    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard
          ...
          spark={<Sparkline points={stats.users.spark} />}
        />
        <KPICard
          icon={<BookOpen size={16} />}
          label="Lessons Published"
          value={stats.lessons.value}
          trend={stats.lessons.trend}
          spark={<Sparkline points={stats.lessons.spark} />}
        />
        <KPICard
          icon={<Trophy size={16} />}
          label="XP Earned (7d)"
          value={stats.xp.value}
          trend={stats.xp.trend}
          spark={<Sparkline points={stats.xp.spark} />}
        />
        <KPICard
          ...
          spark={<Sparkline points={stats.reports.spark} />}
        />
      </section>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-gray-100 bg-white p-4">
            ...
            <div className="mt-3 space-y-3">
              <HealthRow icon={<Cpu size={14} />} label="API Latency" value="112ms" good />
              <HealthRow icon={<Wifi size={14} />} label="Realtime Socket" value="Connected" good />
              <HealthRow icon={<Database size={14} />} label="DB Replication" value="Lag 43ms" good />
            </div>
          </div>
...
function HealthRow({ icon, label, value, good }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-3 py-2">
      <div className="flex items-center gap-2 text-sm text-gray-700">
        <span className={`flex h-6 w-6 items-center justify-center rounded-md ${good ? "bg-[#22C55E]/10 text-[#22C55E]" : "bg-rose-50 text-rose-600"}`}>
          {icon}
        </span>
        {label}
      </div>
